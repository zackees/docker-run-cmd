...
def remove_container(container_name):
    ...
    subprocess.run(
        f"docker rm -f {container_name}", shell=True, check=False, capture_output=True
    )
...
def run_container(
    ...
    run_command = (
        ...
        f'-w "//{working_dir}" {image_name} ' + " ".join(cmd_args)
    )
...
def unit_test() -> None:
    dockerfile = (
        "src/docker_run_cmd/Dockerfile"  # Replace with your actual Dockerfile path
    )
